...
<body>
	...
	<div id = "content">
		...
		<?php
			require_once('class.php');
                          $user = new user();
			$query = $user->runQuery("SELECT * FROM `guide` WHERE `guide_id` = '$_GET[id]'") or die(mysqli_error());
			$fetch = $query->fetch_array();
		?>
			<div class = "panel-heading">
				<label>EDIT guide</label>
				<a href = "guide.php" class = "btn btn-sm btn-info" style = "float:right; margin-top:-5px;"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
			</div>
            ...
            <input type="text" class="form-control" name="name" value = "<?php echo $fetch['gname'] ?>"required data-parsley-pattern="^[a-zA-Z ]+$"/>
            </div>
            <div class="form-group">
			<label for = "age">guide age: </label>
            <input type="text" class="form-control" name="age" value = "<?php echo $fetch['gage'] ?>"required data-parsley-type="number"/>
            </div>
            <div class="form-group">
			<label for = "email">guide email: </label>
            <input type="email" class="form-control" name="email"value = "<?php echo $fetch['gemail'] ?>" required data-parsley-type="email" data-parsley-trigger="keyup" />
            </div>
            <div class="form-group">
			<label for = "phone">guide phone number: </label>
            	<input  class="form-control" name="phone" value = "<?php echo $fetch['gphoneno'] ?>" type = "text" required data-parsley-type="number" required data-parsley-length="[10, 10]" data-parsley-trigger="keyup" />
            </div>
            <div class="form-group">
			<label for = "language">guide known language: </label>
            	<input type="text" class="form-control" name="language" value = "<?php echo $fetch['knownlang'] ?>" required data-parsley-pattern="^[a-zA-Z ]+$"/>
            </div>
            <div class="form-group">
			<label for = "experience">guide experience: </label>
            	<input type="text" class="form-control" name="experience" value = "<?php echo $fetch['exprience'] ?>"required data-parsley-type="alphanum" />
            </div>
            <div class="form-group">
			<label for = "about">about guide: </label>
            	<input type="text" class="form-control" name="about" value = "<?php echo $fetch['about'] ?>"required data-parsley-pattern="^[a-zA-Z ]+$" />
            </div>
							<button  class = "btn btn-warning" name = "edit_guide" ><span class = "glyphicon glyphicon-edit"></span> SAVE</button>
							<br />
					</div>
