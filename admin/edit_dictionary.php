...
<body>
	...
	<div id = "content">
		...
		<?php
			require_once('class.php');
                          $user = new user();
			$query = $user->runQuery("SELECT * FROM `dictionary` WHERE `id` = '$_GET[id]'") or die(mysqli_error());
			$fetch = $query->fetch_array();
		?>
			<div class = "panel-heading">
				<label>EDIT dictionary`</label>
				<a href = "dictionary.php" class = "btn btn-sm btn-info" style = "float:right; margin-top:-5px;"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
			</div>
				...
				<form id = "form_admin" method = "POST" enctype = "multi-part/form-data" >
					...
					<div class = "panel-body">
						<div class = "form-inline">
							...
							<input type="text" name="English" value="<?php echo $fetch['English'];?>" />
							<label for = "Amharic">Amharic: </label>
							<input type="text" name="Amharic" value="<?php echo $fetch['Amharic'];?>"  />
						</div>
							<button  class = "btn btn-warning" name = "edit_dictionary" ><span class = "glyphicon glyphicon-edit"></span> SAVE</button>
							<br />
					</div>
