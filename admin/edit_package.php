...
<body>
	...
	<div id = "content">
		...
		<?php
			require_once('class.php');
                          $user = new user();
			$query = $user->runQuery("SELECT * FROM `package` WHERE `package_id` = '$_GET[id]'") or die(mysqli_error());
			$fetch = $query->fetch_array();
		?>
			<div class = "panel-heading">
				<label>EDIT PACKAGE</label>
				<a href = "package.php" class = "btn btn-sm btn-info" style = "float:right; margin-top:-5px;"><span class = "glyphicon glyphicon-hand-right"></span> BACK</a>
			</div>
            ...
            <input type="text" class="form-control" name="pn" value = "<?php echo $fetch['packagename']?>"  required data-parsley-pattern="^[a-zA-Z ]+$"data-parsley-trigger="keyup"/>
            </div>
            <div class="form-group">
			<label for = "duration">package rate:</label>
            <input type="text" class="form-control" name="pr" value = "<?php echo $fetch['packagerate']?>"required data-parsley-type="number" />
            </div>
            <div class="form-group">
			<label for = "duration">about package:</label>
            	<TextArea type="input" class="form-control" name="pd" value = "<?php echo $fetch['packagedetails']?>"required data-parsley-pattern="^[a-zA-Z ]+$"data-parsley-trigger="keyup" /></TextArea>
            </div>
							<button  class = "btn btn-warning" name = "edit_package" ><span class = "glyphicon glyphicon-edit"></span> SAVE</button>
							<br />
					</div>
