...
<body>
	...
	<div id = "content">
		...
		<div id = "add" class = "panel panel-success">	
            ...
            <input type="text" class="form-control" name="pn" placeholder="enter package name"  required data-parsley-pattern="^[a-zA-Z ]+$"data-parsley-trigger="keyup"/>
            </div>
            <div class="form-group">
			<label for = "duration">package rate:</label>
            <input type="text" class="form-control" name="pr" placeholder="Enter package rate"required data-parsley-type="number"data-parsley-trigger="keyup" />
            </div>
            <div class="form-group">
			<label for = "duration">about package:</label>
            	<TextArea type="input" class="form-control" name="pd" placeholder="Enter package details"required data-parsley-pattern="^[a-zA-Z ]+$"data-parsley-trigger="keyup" /></TextArea>
            </div>
			 <div class="form-group">
			<label for = "pf">package fare:</label>
            <input type="text" class="form-control" name="pf" placeholder="Enter package rate"required data-parsley-type="number"data-parsley-trigger="keyup" />
            </div>
			 <div class="form-group">
			<label for = "city">city:</label>
            <input type="text" class="form-control" name="pc" placeholder="Enter package rate"required data-parsley-type="number"data-parsley-trigger="keyup" />
            </div>
            ...
            <input type="file" class="form-control" name="user_image" required />
							...
							<button  class = "btn btn-primary" name = "save_package" ><span class = "glyphicon glyphicon-save"></span> SAVE</button>
							<br />
					</div>
					<?php require 'add_package.php' ?>					
					</div>
				</form>
			</div>	
		</div>	
			...
			<div class = "panel-body">
				...
				<table id = "table" class = "display" cellspacing = "0"  >
					...
					<?php
						require_once('class.php');
                          $user = new user();
						$query = $user->runQuery("SELECT * FROM `package` ORDER BY `package_id` DESC") or die(mysqli_error());
						while($fetch = $query->fetch_array()){
					?>
						<tr>
							<td><?php echo $fetch['packagename']?></td>
							<td><?php echo md5($fetch['packagerate'])?></td>
							<td><?php echo $fetch['packagedetails']?></td>
							<td><center><a class = "btn btn-sm btn-warning" href = "edit_package.php?id=<?php echo $fetch['package_id']?>&packagename=<?php echo $fetch['packagename']?>"><i class = "glyphicon glyphicon-edit"></i> Update</a> <a onclick="confirmationDelete(this);return false;" href = "delete_package.php?id=<?php echo $fetch['package_id']?>" class = "btn btn-sm btn-danger"><i class = "glyphicon glyphicon-remove"></i> Delete</a></center></td>
						</tr>
					<?php
						}
